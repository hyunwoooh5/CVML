...
if type(g1) == CV_MLP:
    ...
    j = jax.jit(jax.jacfwd(lambda x, p: g1.apply(p, x, shape)[0], argnums=0))
...
for i in range(nt):
    for j in range(nt):
        cov[i*nt+j] = (obs[:, i]-avs[i])@(obs[:, j]-avs[j]) / \
            (args.n_test+args.n_train-1)/(args.n_test+args.n_train)
...
for i in range(nt):
    for j in range(nt):
        cov_full[i*nt+j] = (obs_full[:, i]-avs_full[i])@(obs_full[:,
                                                                  j]-avs_full[j])/(len(obs_full)-1)/len(obs_full)
...
for i in range(nt):
    for j in range(nt):
        cov_cv[i*nt+j] = (obs_cv[:args.n_test, i]-avs_cv[i]
                          )@(obs_cv[:args.n_test, j]-avs_cv[j])/(args.n_test-1)/args.n_test
print(f"covariance:")
for i in range(nt):
...
jack_cv_all = [0]*nt
cov_cv_all = [0] * (nt**2)
obs_cv_all = obs[:args.n_test] - \
    jnp.concatenate([cv[:args.n_test]]*nt).reshape([nt, args.n_test]).T
...
for i in range(nt):
    for j in range(nt):
        cov_cv_all[i*nt+j] = (obs_cv_all[:, i]-avs_cv_all[i]
                              )@(obs_cv_all[:, j]-avs_cv_all[j])/(args.n_test-1)/args.n_test
print(f"covariance:")
for i in range(nt):
...
for i in range(nt):
    for j in range(nt):
        cov_optimized[i*nt+j] = (obs_optimized[:, i]-avs_optimized[i])@(
            obs_optimized[:, j]-avs_optimized[j])/(args.n_test-1)/(args.n_test)
print(f"covariance:")
for i in range(nt):
...
for i in range(nt):
    for j in range(nt):
        cov_cv_f[i*nt+j] = (obs_cv_f[:, i]-avs_cv_f[i])@(obs_cv_f[:, j] -
                                                         avs_cv_f[j])/(args.n_test+args.n_train-1)/(args.n_test+args.n_train)
print(f"covariance:")
for i in range(nt):
...
cov_cv_all_f = [0] * (nt**2)
obs_cv_all_f = obs - \
    jnp.concatenate([cv]*nt).reshape([nt, args.n_test+args.n_train]).T
...
for i in range(nt):
    for j in range(nt):
        cov_cv_all_f[i*nt+j] = (obs_cv_all_f[:, i]-avs_cv_all_f[i])@(obs_cv_all_f[:, j] -
                                                                     avs_cv_all_f[j])/(args.n_test+args.n_train-1)/(args.n_test+args.n_train)
print(f"covariance:")
for i in range(nt):
...
for i in range(nt):
    for j in range(nt):
        cov_optimized_f[i*nt+j] = (obs_optimized_f[:, i]-avs_optimized_f[i])@(
            obs_optimized_f[:, j]-avs_optimized_f[j])/(args.n_test+args.n_train-1)/(args.n_test+args.n_train)
print(f"covariance:")
for i in range(nt):
