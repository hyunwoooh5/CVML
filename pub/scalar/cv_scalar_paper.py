...
if __name__ == '__main__':
    ...
    parser.add_argument('--cnn',  action='store_true',
                        help="Use CNN")
    parser.add_argument('-ob', '--observe', type=int, default=0, help="correlator")
    ...
    if not loaded:
        if args.cnn:
            ...
            def Loss(x, p):
                ...
                return jnp.abs(model.observe(x, args.observe) - f(x, p) - y[0])**2 + sum(l2_loss(w, alpha=args.l2) for w in jax.tree_util.tree_leaves(p["params"])) - args.l2 * y[0]**2
    ...
    obs = jax.vmap(lambda y: model.observe(y, args.observe))(configs_test)
